...
def __maybe_download_file(destination: str, source: str):
    ...
    if not os.path.exists(destination):
        logger.info(f"{destination} does not exist. Downloading ...")
        # urllib.request.urlretrieve(source, filename=destination + '.tmp')
        # os.rename(destination + '.tmp', destination)
        logger.info(f"Downloaded {destination}.")
    else:
        logger.info(f"Destination {destination} exists. Skipping.")
    return destination
...
def __extract_all_files(filepath: str, data_root: str, data_dir: str):
    if not os.path.exists(data_dir):
        ...
        logger.info(f'Skipping extracting. Data already there {data_dir}')


def extract_file(filepath: str, data_dir: str):
    try:
        with tarfile.open(filepath) as tar:
            # Define a simple filter that allows all files
            def all_files(member):
                return member

            tar.extractall(path=data_dir, members=safe_members(tar))
    except Exception as e:
        logger.info(f'Not extracting. Maybe already there? Exception: {e}')

def safe_members(tar):
    """Filter function to process members before extracting."""
...
def main():
    ...
    data_set = "gsc_v{0}".format(args.data_version)
